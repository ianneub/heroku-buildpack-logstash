...
class LogStash::Inputs::S3 < LogStash::Inputs::Base
  ...
  def sincedb_read()
    ...
    @logger.info("reading since from s3")
    if @s3bucket.objects['sincedb'].exists?
      since = Time.parse(@s3bucket.objects['sincedb'].read.chomp.strip)
      @logger.info("read #{since.to_s} from s3 bucket")
    else
      @logger.info("s3 bucket not found so returning since time of now")
      since = Time.new(0)
    end
